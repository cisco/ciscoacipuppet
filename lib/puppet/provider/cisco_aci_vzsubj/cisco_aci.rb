...
#
# Puppet resource provider for vzsubj
...
Puppet::Type.type(:cisco_aci_vzsubj).provide(:cisco_aci) do
  desc 'The Cisco provider for vzsubj.'
  ...
  def self.mo_class_query_url
    '/api/class/vzSubj.json?query-target=self'
  end
  ...
  def self.pt_attr_2_mo_attr
    {
      cons_match_t:  'consMatchT',
      descr:         'descr',
      dn:            'dn',
      name:          'name',
      name_alias:    'nameAlias',
      prio:          'prio',
      prov_match_t:  'provMatchT',
      rev_flt_ports: 'revFltPorts',
      target_dscp:   'targetDscp',
    }
  end
  ...
  def self.prefetch(resources)
    vzsubj_instances = instances
    resources.keys.each do |name|
      provider = vzsubj_instances.find do |inst|
        inst.name.to_s == resources[name][:name].to_s &&
        inst.fvtenant.to_s == resources[name][:fvtenant].to_s &&
        inst.vzbrcp.to_s == resources[name][:vzbrcp].to_s &&
        inst
      end
