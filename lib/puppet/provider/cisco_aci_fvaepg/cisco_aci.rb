...
#
# Puppet resource provider for fvaepg
...
Puppet::Type.type(:cisco_aci_fvaepg).provide(:cisco_aci) do
  desc 'The Cisco provider for fvaepg.'
  ...
  def self.mo_class_query_url
    '/api/class/fvAEPg.json?query-target=self'
  end
  ...
  def self.pt_attr_2_mo_attr
    {
      ...
      dn:                 'dn',
      fwd_ctrl:           'fwdCtrl',
      is_attr_based_e_pg: 'isAttrBasedEPg',
      match_t:            'matchT',
      name:               'name',
      name_alias:         'nameAlias',
      pc_enf_pref:        'pcEnfPref',
      pref_gr_memb:       'prefGrMemb',
      prio:               'prio',
    }
  end
  ...
  def self.prefetch(resources)
    fvaepg_instances = instances
    resources.keys.each do |name|
      provider = fvaepg_instances.find do |inst|
        inst.name.to_s == resources[name][:name].to_s &&
        inst.fvtenant.to_s == resources[name][:fvtenant].to_s &&
        inst.fvap.to_s == resources[name][:fvap].to_s &&
        inst
      end
