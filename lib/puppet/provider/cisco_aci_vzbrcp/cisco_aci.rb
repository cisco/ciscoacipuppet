...
#
# Puppet resource provider for vzbrcp
...
Puppet::Type.type(:cisco_aci_vzbrcp).provide(:cisco_aci) do
  desc 'The Cisco provider for vzbrcp.'
  ...
  def self.mo_class_query_url
    "/api/class/vzBrCP.json?query-target=self"
  end
  ...
  def self.prefetch(resources)
    vzbrcp_instances = instances
    resources.keys.each do |name|
      provider = vzbrcp_instances.find do |inst|
