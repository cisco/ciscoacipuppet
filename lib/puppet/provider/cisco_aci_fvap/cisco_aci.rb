...
#
# Puppet resource provider for fvap
...
Puppet::Type.type(:cisco_aci_fvap).provide(:cisco_aci) do
  desc 'The Cisco provider for fvap.'
  ...
  def self.mo_class_query_url
    '/api/class/fvAp.json?query-target=self'
  end
  ...
  def self.prefetch(resources)
    fvap_instances = instances
    resources.keys.each do |name|
      provider = fvap_instances.find do |inst|
