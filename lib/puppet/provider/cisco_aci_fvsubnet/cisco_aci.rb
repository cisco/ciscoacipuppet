...
#
# Puppet resource provider for fvsubnet
...
Puppet::Type.type(:cisco_aci_fvsubnet).provide(:cisco_aci) do
  desc 'The Cisco provider for fvsubnet.'
  ...
  def self.mo_class_query_url
    "/api/class/fvSubnet.json?query-target=self"
  end
  ...
  def self.prefetch(resources)
    fvsubnet_instances = instances
    resources.keys.each do |name|
      provider = fvsubnet_instances.find do |inst|
        inst.ip.to_s == resources[name][:ip].to_s &&
        inst.fvtenant.to_s == resources[name][:fvtenant].to_s &&
        inst.fvbd.to_s == resources[name][:fvbd].to_s &&
        inst
      end
