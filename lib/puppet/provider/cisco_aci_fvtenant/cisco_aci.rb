...
#
# Puppet resource provider for fvtenant
...
Puppet::Type.type(:cisco_aci_fvtenant).provide(:cisco_aci) do
  desc 'The Cisco provider for fvtenant.'
  ...
  def self.mo_class_query_url
    "/api/class/fvTenant.json?query-target=self"
  end
  ...
  def self.prefetch(resources)
    fvtenant_instances = instances
    resources.keys.each do |name|
      provider = fvtenant_instances.find do |inst|
