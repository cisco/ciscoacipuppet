  ...
  module Cisco
    class ACIUtils
      ...
      def self.xtract_attr_hash_frm_json(mo_hash,p_class)
        pt_attrs_hash = {}
        if ((values = mo_hash[p_class.mo_class_name]) and
            (attrs  = values['attributes']))
              p_class.pt_attr_2_mo_attr.each do |pt_attr,mo_attr|
                if attrs[mo_attr]
                  pt_attrs_hash[pt_attr] = attrs[mo_attr]
                end
              end
        end
        #Parent MOs are namevars in Puppet. Exract their value from DN
        dn_p2v_hash = self.xtract_prfx2val_frm_dn(pt_attrs_hash[:dn])
      ...
      def self.gen_rn(keys2value_map,
        ...
        if (my_namevars.length == 1 &&
            my_namevars[0] == "name")
          rn = rn + keys2value_map[name]
        else
          rn = rn + "["
          my_namevars.each { |k| rn = rn + keys2value_map[k]}
          rn = rn + "]"
        end
        rn
      end
      ...
      def self.gen_dn(p_inst)
        ...
        p_inst.class.parent_namevars.each do |key,prefix|
          dn = dn + prefix + "-" + p_inst.property_flush[key] + "/"
        end
        dn = dn + self.gen_rn(p_inst.property_flush,p_inst.class.mo_dn_prefix,
      ...
      def self.gen_json4createmodify(p_inst)
        ...
        attributes.delete_if {|key,value| (key == :ensure ||
                                  key == :provider ||
      ...
      #@param p_inst [Object] Provider instance
      #
      def self.set_property_flush_keys_from_property_hash(p_inst)
      ...
      #@param p_inst [Object] Provider instance
      #
      def self.set_property_flush_attrs_frm_resource(p_inst)
      ...
      def self.puppet_hash_init(provider_inst)
        self.set_property_flush_keys_from_property_hash(provider_inst)
      ...
      def self.create(provider_inst)
        self.set_property_flush_attrs_frm_resource(provider_inst)
      end
      ...
      def self.flush(provider_inst)
        provider_inst.property_flush[:dn] = self.gen_dn(provider_inst)
        begin
          case provider_inst.property_flush[:ensure]
          when :present
            resp = Puppet::Util::NetworkDevice.current.post(
                    self.gen_url4instance(provider_inst.property_flush[:dn]),
                    self.gen_json4createmodify(provider_inst))
            resp["imdata"].each do |mo|
              resp_attrs_hash = self.xtract_attr_hash_frm_json(mo,
                                                          provider_inst.class)
              self.debug_what_changed(resp_attrs_hash,
                                      provider_inst.property_hash)
            end
          when :absent
            resp = Puppet::Util::NetworkDevice.current.delete(
                      self.gen_url4instance(provider_inst.property_flush[:dn]))
          end
          #Return the corresponding APIC GUI URI that will be used for informational
          #message
          self.gen_url4apicgui(provider_inst.property_flush[:fvtenant],
      ...
      def self.xtract_fabric_member_frm_dhcp_query(one_dhcp_clnt_json)
        if (one_dhcp_clnt_json and
          (values = one_dhcp_clnt_json['dhcpClient']) and
          (attrs  = values['attributes']) and
          (id = attrs['id']) and
          ((attrs['nodeRole'] == 'leaf') or (attrs['nodeRole'] == 'spine')))
      ...
      def self.get_facts()
        ...
        mem_lst['imdata'].each do |mem|
          mem_attrs = self.xtract_fabric_member_frm_dhcp_query(mem)
